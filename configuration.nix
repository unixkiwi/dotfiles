  ...
  system.activationScripts.flatpakManagement = {
    text = let
      ...
      for installed in $installedFlatpaks; do
        if ! echo ${toString flats} | ${grep}/bin/grep -q $installed; then
          echo "[FlatpakAutoLOG] Removing $installed"
          ${pkgs.flatpak}/bin/flatpak uninstall -y --noninteractive $installed
        fi
      done
      
      for app in ${toString flats}; do
        echo "[FlatpakAutoLOG] Installing $app..."
        ${pkgs.flatpak}/bin/flatpak install -y flathub $app
      done

      echo "[FlatpakAutoLOG] Updating flats..."
      ${pkgs.flatpak}/bin/flatpak update -y

      echo "[FlatpakAutoLOG] Finished FlatpakAutoStartScript!"
    '';
  };
